...
class ArrayToXML {
    ...
    private static function &convert($node_name, $arr=array()) {
        ...
        if(is_array($arr)){
            ...
            if(isset($arr['@attributes'])) {
                foreach($arr['@attributes'] as $key => $value) {
                    ...
                    $node->setAttribute($key, self::booltostr($value));
                }
            ...
            if(isset($arr['@value'])) {
                $node->appendChild($xml->createTextNode(self::booltostr($arr['@value'])));
                unset($arr['@value']);    //remove the key from the array once done.
                //return from recursion, as a note with value cannot have child nodes.
                return $node;
            } else if(isset($arr['@cdata'])) {
                $node->appendChild($xml->createCDATASection(self::booltostr($arr['@cdata'])));
        ...
        if(!is_array($arr)) {
            $node->appendChild($xml->createTextNode(self::booltostr($arr)));
        }
     ...
     */
    private static function booltostr($v){
